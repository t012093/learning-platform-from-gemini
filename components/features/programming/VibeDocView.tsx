...
import { ViewState, DocChapter, LocalizedDocBlock, QuizData } from '../../../types';
  ...
  return (
    <div className="min-h-screen bg-white text-slate-800 font-sans">
      ...
      <header className={`fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-md border-b border-slate-100 z-50 flex items-center justify-between px-6 transition-transform duration-300 ${viewMode === 'slide' ? '-translate-y-full hover:translate-y-0' : ''}`}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm font-bold text-slate-800 line-clamp-1">{chapter.title[language]}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
           <span className="text-xs font-medium text-slate-400 flex items-center gap-1">
             <Clock size={14} /> {chapter.readingTime[language]}
           </span>
        </div>
      </header>
             ...
             <ul className="space-y-1 relative border-l border-slate-100">
                {chapter.sections.map((section) => (
                  <li key={section.id} className="relative">
                     ...
                     >
                        {(section.title[language].split('. ')[1] || section.title[language])}
                     </button>
                  </li>
                ))}
             </ul>
      ...
      {viewMode === 'slide' && pdfUrl && (
         <SlideViewer pdfUrl={pdfUrl} onBack={() => setViewMode('doc')} language={language} />
      )}
...
const BlockRenderer: React.FC<{ block: LocalizedDocBlock; language: 'en' | 'jp' }> = ({ block, language }) => {
   switch (block.type) {
      ...
      case 'text':
         return (
            <p className={`leading-8 text-slate-700 ${block.style === 'lead' ? 'text-xl font-light text-slate-600 mb-8' : 'text-base'} ${block.style === 'quote' ? 'font-serif text-lg italic text-slate-600 border-l-4 border-slate-200 pl-4 py-1' : ''}`}>
               {block.text[language]}
            </p>
         );
         ...
         return (
            <div className={`my-8 p-6 rounded-xl border ${style.bg} ${style.border} flex gap-4`}>
               ...
               <div>
                  ...
                  <p className={`text-sm leading-relaxed ${style.text}`}>{block.text[language]}</p>
               </div>
            </div>
         );
            ...
            <ul className={`my-6 space-y-3 ${block.style === 'number' ? 'list-decimal pl-5' : ''}`}>
               {block.items.map((item, i) => (
                  <li key={i} className="flex gap-3 text-slate-700 leading-relaxed group">
                     ...
                     <span dangerouslySetInnerHTML={{ __html: item[language].replace(/\*\*(.*?)\*\*/g, '<strong class="text-slate-900 font-bold">$1</strong>') }} />
                  </li>
               ))}
            </ul>
         );
